...
function npOnly(req: Request, res: Response, next: NextFunction) {
  ...
  if (!user || user.role !== "np") {
    return res
      .status(403)
      .json({ error: "Access restricted to Nurse Practitioners" });
  }
  next();
}
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (allowed.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          "File type not allowed. Accepted: PDF, JPG, PNG, WEBP, GIF, DOC, DOCX",
        ),
      );
    }
  },
...
If someone asks something completely unrelated to women's health or menopause, politely redirect them.`;
  ...
  app.get(
    ...
    async (_req: Request, res: Response) => {
      try {
        return res.json(await getAllQuizSubmissions());
      } catch (err) {
        return res
          .status(500)
          .json({ error: "Failed to get quiz submissions" });
      }
    },
  );
  ...
  app.get(
    ...
    async (_req: Request, res: Response) => {
      try {
        return res.json(await getAllFollowUpRequests());
      } catch (err) {
        return res
          .status(500)
          .json({ error: "Failed to get follow-up requests" });
      }
    },
  );
  ...
  app.get(
    ...
    async (_req: Request, res: Response) => {
      try {
        return res.json(await getAllWaitlist());
      } catch (err) {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res.json({
          ...
          assignedNP: assignedNP
            ? {
                id: assignedNP.id,
                firstName: assignedNP.firstName,
                lastName: assignedNP.lastName,
                email: assignedNP.email,
              }
            : null,
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ error: "Failed to load treatment plans" });
      }
    },
  );
  ...
  app.get(
    "/api/patient/messages/:partnerId",
    portalAuth,
    patientOnly,
    async (req: Request, res: Response) => {
      try {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ error: "Failed to load patient profile" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ error: "Failed to create treatment plan" });
      }
    },
  );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (req.body.followUpDate)
          updates.followUpDate = new Date(req.body.followUpDate);
        await updateTreatmentPlan(planId, updates);
        return res.json({ success: true });
      } catch (err) {
        return res
          .status(500)
          .json({ error: "Failed to update treatment plan" });
      }
    },
  );
  ...
  app.patch(
    "/api/np/treatment-plans/:id/status",
    portalAuth,
    npOnly,
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ error: "Failed to update treatment plan status" });
      }
    },
  );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!patientId || !title || !scheduledAt) {
          return res
            .status(400)
            .json({ error: "Patient, title, and scheduled time are required" });
        }
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ error: "Failed to mark document as reviewed" });
      }
    },
  );
  ...
  // NP — Clinical Notes
  app.get(
    "/api/np/clinical-notes/:patientId",
    portalAuth,
    npOnly,
    async (req: Request, res: Response) => {
      try {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!patientId || !content)
          return res
            .status(400)
            .json({ error: "Patient ID and content are required" });
        await createClinicalNote({ patientId, npId, content });
        return res.json({ success: true });
      } catch (err) {
        return res
          .status(500)
          .json({ error: "Failed to create clinical note" });
      }
    },
  );

  app.put(
    "/api/np/clinical-notes/:id",
    portalAuth,
    npOnly,
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ error: "Failed to update clinical note" });
      }
    },
  );
