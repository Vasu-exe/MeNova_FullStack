...
export async function getAllNPs() {
  ...
  return db
    .select()
    .from(portalUsers)
    .where(eq(portalUsers.role, "np"))
    .orderBy(portalUsers.firstName);
}
...
export async function getAllPatients() {
  ...
  return db
    .select()
    .from(portalUsers)
    .where(eq(portalUsers.role, "patient"))
...
export async function getQuizSubmissionsByPatientId(patientId: number) {
  ...
  return db
    .select()
    .from(quizSubmissions)
    .where(eq(quizSubmissions.portalUserId, patientId))
    .orderBy(desc(quizSubmissions.createdAt));
}
...
export async function resolveFollowUpRequest(
  sessionId: string,
  qualified: boolean,
) {
  const db = await getDb();
...
export async function getAdminStats() {
  ...
  const [qualifiedCount] = await db
    .select({ count: sql<number>`count(*)` })
    .from(followUpRequests)
    .where(eq(followUpRequests.status, "qualified"));
  const [pendingCount] = await db
    .select({ count: sql<number>`count(*)` })
    .from(followUpRequests)
    .where(eq(followUpRequests.status, "pending"));
  const [patientCount] = await db
    .select({ count: sql<number>`count(*)` })
    .from(portalUsers)
    .where(eq(portalUsers.role, "patient"));
  const [npCount] = await db
    .select({ count: sql<number>`count(*)` })
    .from(portalUsers)
    .where(eq(portalUsers.role, "np"));
...
export async function getAppointmentsByNP(npId: number) {
  ...
  return db
    .select()
    .from(appointments)
    .where(eq(appointments.npId, npId))
    .orderBy(desc(appointments.scheduledAt));
}
...
export async function getConversationsForUser(userId: number) {
  ...
  const sent = await db
    .select({ partnerId: messages.receiverId })
    .from(messages)
    .where(eq(messages.senderId, userId));
  const received = await db
    .select({ partnerId: messages.senderId })
    .from(messages)
  ...
  const partners = await db
    .select()
    .from(portalUsers)
    .where(inArray(portalUsers.id, partnerIds));
  return partners;
}
...
export async function getAssignedPatients(npId: number) {
  ...
  const activeAssignments = await db
    ...
    .where(
      and(
        eq(patientAssignments.npId, npId),
  ...
  return db
    .select()
    .from(portalUsers)
    .where(inArray(portalUsers.id, patientIds));
}
